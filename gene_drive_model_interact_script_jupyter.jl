...
#
	...
	using Measurements, LightXML, Statistics, ArgParse
	using Pkg
...
run_model_pre = function(; pop_input = 100000, pops_input = 1, indiv_input = 3, gens_input = 100, fis_input = 0.0, migration_input = false, exposure1 = 1.0, exposure2 = 1.0,
                       dominance_input = 0.0, fitness_input = 0.0, conversion_input = 1.0, resistance_input = 0.0, drive_freq = 0.001, resfreq_input1 = 0.1, resfreq_input2 = 0.1,
...
for iter in 1:iterations_input
    ...
    if sum(freq1 .< 0) > 0
        h1 = Plots.plot()
        annotate!(0.5, 0.5, "Sum of frequencies greater than 1", :red)
        return(h1)
        break
    end
...
mp = @manipulate throttle= 0.1 for  zygotic       = Widgets.widget(Dict("Post-Zygotic" => 1, "Pre-Zygotic" => 2), label="Timing"),
                                    ...
                                    fitnesscost   = Widgets.slider((0:0.01:1.0), value = 0.0, label = "Selection Coefficient"),
                                    exposurerate1 = Widgets.slider((0:0.01:1.0), value = 0.0, label = "Exposure Rate 1"),
                                    exposurerate2 = Widgets.slider((0:0.01:1.0), value = 0.0, label = "Exposure Rate 2"),
                                    ...
                                    drivefreq     = Widgets.slider((0:0.01:1.0), value = 0.0, label = "Drive Frequency"),
                                    resfreq1      = Widgets.slider((0:0.01:1.0), value = 0.0, label = "Resistance Frequency 1"),
                                    resfreq2      = Widgets.slider((0:0.01:1.0), value = 0.0, label = "Resistance Frequency 2"),
                                    migrants      = Widgets.slider((-1.0:-1.0:-5.0), value = 0.0, label = "Migration Rate"),
                                    migon         = Widgets.widget(Dict("Off" => 0, "On  " => 1), label="Migration"),
                                    rerun         = Widgets.widget(Dict("No   " => 1, "Yes" => 2), label="Refresh"),
                                    iterations    = Widgets.slider((0:1:1), value = 0.0, label = "10 ^ Iterations")
        if rerun == 2
            # println("hello")
            include("gene_drive_model_interact_script.jl")
        end
...
@layout! mp vbox(hbox(pad(1em, :zygotic), pad(1em, :migon)),
					hbox(
                 	  vbox(vbox(:popsi, :convi, :fitnesscost, :resistance, :dominance)),
				 	  vbox(vbox(:popi, :inbreeding, :exposurerate1, :resfreq1, :drivefreq)),
				 	  vbox(vbox(:gensi, :migrants, :exposurerate2, :resfreq2, :iterations))),
